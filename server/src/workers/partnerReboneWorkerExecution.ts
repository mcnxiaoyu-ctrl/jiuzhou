 ...
 *
 * 作用（做什么 / 不做什么）：
 * 1) 做什么：统一封装 worker 真正执行洗髓任务前的动态伙伴快照同步与结果映射。
 * 2) 做什么：让消息监听层只负责收发协议，避免“先刷新快照再执行业务”的关键顺序散落在事件回调里。
 * 3) 不做什么：不管理线程生命周期，不处理主线程推送，也不改数据库事务边界。
 *
 * 输入/输出：
 * - 输入：`reboneId`、`characterId`，以及可注入的快照刷新/业务处理依赖。
 * - 输出：标准化 `PartnerReboneWorkerResponse`。
 *
 * 数据流/状态流：
 * worker payload -> 刷新 generated_partner_def 快照 -> processPendingReboneJob -> worker result response。
 *
 * 关键边界条件与坑点：
