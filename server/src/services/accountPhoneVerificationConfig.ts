 ...
 *
 * 作用（做什么 / 不做什么）：
 * 1. 做什么：集中解析账号级手机号验证码开关、短信模板与发送频控配置，供注册、手机号登录、老账号绑定和坊市绑定共用。
 * 2. 做什么：把原先散落在坊市命名下的短信能力提升为账号级能力，避免登录注册与坊市绑定各自读取环境变量。
 ...
 *
 * 数据流/状态流：
 * 环境变量 -> 本模块归一化 -> 短信服务 / 手机号验证码服务 / 坊市绑定服务复用。
 *
 * 复用设计说明：
 * - 登录、注册、老账号绑定、坊市绑定都依赖同一套短信模板和发送限流，集中到这里可以避免多个配置模块重复判断。
