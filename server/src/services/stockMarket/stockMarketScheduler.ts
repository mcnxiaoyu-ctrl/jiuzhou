 ...
 *
 * 作用（做什么 / 不做什么）：
 * 1. 做什么：在后台调度角色中按整点触发股市 AI 新闻与行情 tick。
 * 2. 不做什么：不实现 AI prompt、不拼交易 SQL、不接入 cleanupWorker。
 *
 * 输入 / 输出：
 * - 输入：启动流水线调用的 initialize/stop。
 * - 输出：定时触发 `stockMarketService.runHourlyTick`，并记录日志。
 *
 * 数据流 / 状态流：
 * startupPipeline -> initializeStockMarketHourlyScheduler -> setTimeout 到下一整点 -> runHourlyTick -> 重新安排下一轮。
 *
 * 复用设计说明：
 * - 调度生命周期和排行夜间刷新保持同一模式，启动、互斥、停止都在一个模块中收敛。
 * - 整点计算复用 `stockMarketTime`，概览倒计时和后台触发不会各自计算。
 *
 * 关键边界条件与坑点：
 * 1. 不能使用固定 setInterval，否则服务重启或执行耗时会让每小时 tick 逐渐漂移。
...
const runScheduledStockMarketTick = async (): Promise<void> => {
  ...
  inFlight = true;
  try {
    console.log('[StockMarketScheduler] 开始生成每小时股市新闻与行情');
    const result = await stockMarketService.runHourlyTick(new Date());
    console.log(`[StockMarketScheduler] ${result.message}`);
  } catch (error) {
    console.error('[StockMarketScheduler] 股市每小时行情执行失败:', error);
  } finally {
    inFlight = false;
    if (initialized) {
...
export const initializeStockMarketHourlyScheduler = async (): Promise<void> => {
...
export const stopStockMarketHourlyScheduler = (): void => {
  initialized = false;
  inFlight = false;
  clearScheduledTimer();
};
