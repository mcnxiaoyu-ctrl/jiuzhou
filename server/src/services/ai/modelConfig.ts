 ...
 * 2) 做什么：给功法文本生成、伙伴文本生成、云游奇遇、股市新闻、技能图标、伙伴头像等 AI 调用链提供单一配置入口，避免每个业务模块各自解析环境变量。
...
export type TextModelScope = 'technique' | 'partner' | 'wander' | 'stockMarket';
...
const TEXT_MODEL_ENV_KEYS: Record<TextModelScope, {
  ...
  stockMarket: {
    provider: 'AI_STOCK_MARKET_MODEL_PROVIDER',
    url: 'AI_STOCK_MARKET_MODEL_URL',
    key: 'AI_STOCK_MARKET_MODEL_KEY',
    name: 'AI_STOCK_MARKET_MODEL_NAME',
  },
};
