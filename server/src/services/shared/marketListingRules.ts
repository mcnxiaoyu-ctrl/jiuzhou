 ...
 * 2. 做什么：提供过期时间计算与过期判断，避免服务层、RabbitMQ 队列和静态测试各自维护魔法数字。
 ...
 * - RabbitMQ 延迟队列读取同一时长声明 TTL，消费者复用同一时长计算到期 now。
 *
 * 复用设计说明：
 * - 上架规则属于高频业务变化点，放在单一模块后，服务校验、SQL 过滤和 RabbitMQ 延迟队列不会重复散落数字。
 * - 当前被 `marketService`、`marketListingAutoCancelService` 与规则静态测试复用。
 *
 * 关键边界条件与坑点：
 * 1. “每日”口径由调用方使用上海自然日工具生成窗口，本模块只维护次数上限，避免时区职责混在一起。
 * 2. 过期判断使用毫秒差值，必须和 RabbitMQ TTL 的小时数常量保持同源。
 */
