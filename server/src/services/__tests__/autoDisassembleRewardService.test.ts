...
test('非装备批量命中规则时应一次性累计银两奖励', async () => {
  const { calls, fn } = createCreateItemMock([]);
  const silverCalls: number[] = [];
  ...
  assert.deepEqual(result.grantedItems, []);
  assert.deepEqual(result.pendingMailItems, []);
  assert.equal(result.gainedSilver, 54);
  assert.deepEqual(silverCalls, [54]);
...
test('功法书批量命中规则时应一次性创建聚合后的残页奖励', async () => {
  const { calls, fn } = createCreateItemMock([
    {
      success: true,
      message: 'ok',
  ...
  assert.deepEqual(result.warnings, []);
  assert.deepEqual(result.pendingMailItems, []);
  assert.deepEqual(result.grantedItems, [{ itemDefId: 'mat-gongfa-canye', qty: 30, itemIds: [811] }]);
  assert.equal(result.gainedSilver, 0);
  assert.equal(calls.length, 1);
  assert.equal(calls[0]?.itemDefId, 'mat-gongfa-canye');
  assert.equal(calls[0]?.qty, 30);
});
