...
test('buildWanderAiEpisodeSetupSystemMessage: 终幕待选幕次必须禁止提前写尾声与称号', () => {
  ...
  assert.match(systemMessage, /player\.storyPartner/u);
...
test('buildWanderAiEpisodeSetupUserPayload: 不应再把 mainQuestName 透传给模型，且应暴露终幕待选标记', () => {
  ...
  assert.equal(payload.player.storyPartner, null);
  assert.equal(payload.storyLocation.fullName, '东洲·青云村外·南侧密林');
});

test('buildWanderAiEpisodeSetupUserPayload: 前文上下文与故事级伙伴快照必须完整透传', () => {
  const payload = buildWanderAiEpisodeSetupUserPayload({
    nickname: '测试角色',
    realm: '炼气期',
    hasTeam: false,
  ...
  assert.equal(payload.player.storyPartner?.nickname, '小瓯');
  assert.equal(payload.player.storyPartner?.role, '辅助');
});
...
test('buildWanderAiEpisodeResolutionUserPayload: 应透传本幕选择与终幕结算模式', () => {
  ...
  assert.equal(payload.player.storyPartner?.nickname, '小瓯');
  assert.equal(payload.player.storyPartner?.quality, '紫');
});
