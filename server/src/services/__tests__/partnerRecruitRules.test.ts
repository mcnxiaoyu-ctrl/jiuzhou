...
test('resolvePartnerRecruitQualityForGeneratedPreviewSuccess: 首次保底玄级只影响实际产出，不改变展示概率表', () => {
  ...
  try {
    ...
    assert.equal(resolvePartnerRecruitQualityForGeneratedPreviewSuccess(0, 'production', '玄'), '玄');
  } finally {
...
test('rollPartnerRecruitPrimaryAttackGrowthTarget: 高值出现概率应低于低值', () => {
  ...
  assert.equal(total / 2000 < 27.5, true);
});
...
test('rollPartnerRecruitPrimaryAttackGrowthTarget: 天级 45+ 应为少见长尾', () => {
  let count45Plus = 0;
