...
import { recordKillMonsterEvents } from './taskService.js';
  ...
  try {
    if (task.payload.dungeonStartConsumption) {
      await settleDungeonStartConsumptionInDb({
        settlementKey: task.taskId,
        payload: task.payload.dungeonStartConsumption,
      });
    ...
    if (task.payload.battleType === 'pve') {
      ...
      if (task.payload.dungeonSettlement && task.payload.result === 'attacker_win') {
        const result = await settleDungeonClearInDb({
          participants: task.payload.participants,
          rewardParticipants: task.payload.rewardParticipants,
          dungeonSettlement: task.payload.dungeonSettlement,
        });
