 ...
 *
 * 作用（做什么 / 不做什么）：
 * 1. 做什么：集中处理登录、注册、老账号绑定三类未登录态短信验证码发送、图片/天御人机验证和倒计时。
 * 2. 做什么：把发码请求、local 图片验证码刷新、腾讯天御弹窗触发收敛到单一 Hook，避免三个表单各自拼请求和维护倒计时。
 ...
 *
 * 复用设计说明：
 * - 三个鉴权表单共享同一发码链路，Hook 保证验证码 provider、冷却倒计时和错误后的刷新策略只有一个入口。
