...
import { useMarketPurchaseCaptchaConfig } from './useMarketPurchaseCaptchaConfig';
import {
...
type MarketPurchaseCaptchaTicket = Pick<TencentCaptchaVerifyPayload, 'ticket' | 'randstr'>;
type MarketPurchaseCaptchaPayloadResolution =
  | { proceed: true; captchaPayload?: MarketPurchaseCaptchaTicket }
  | { proceed: false };
          ...
          <div className="market-list-sheet-meta">
            ...
            <div className="market-list-sheet-tags">
              <span className="market-list-sheet-tag market-list-sheet-tag--cat">
                ...
                <span className={`market-list-sheet-tag market-list-sheet-tag--quality ${getItemQualityTagClassName(item.quality)}`}>
...
const MarketModal: React.FC<MarketModalProps> = ({ open, onClose, playerName = '我' }) => {
  ...
  const {
    ...
    loading: marketPurchaseCaptchaConfigLoading,
  } = useMarketPurchaseCaptchaConfig(marketModalOpen);
  const {
  ...
  const resolveMarketPurchaseCaptchaPayload = useCallback(async (): Promise<MarketPurchaseCaptchaPayloadResolution> => {
    if (marketPurchaseCaptchaConfigLoading || marketPurchaseCaptchaSdkLoading) {
      messageRef.current.error('购买验证准备中，请稍后重试');
      return { proceed: false };
    }
    if (!marketPurchaseCaptchaConfig.enabled || !marketPurchaseCaptchaConfig.tencentAppId) {
      return { proceed: true };
    }

    try {
      const captchaPayload = await triggerMarketPurchaseCaptcha();
      return { proceed: true, captchaPayload };
    } catch (error) {
      if (!isTencentCaptchaCancelledError(error)) {
        const normalizedError = toUnifiedApiError(error, '购买验证失败');
        messageRef.current.error(normalizedError.message);
      }
      return { proceed: false };
    }
  }, [
    marketPurchaseCaptchaConfig.enabled,
    marketPurchaseCaptchaConfig.tencentAppId,
    marketPurchaseCaptchaConfigLoading,
    marketPurchaseCaptchaSdkLoading,
    triggerMarketPurchaseCaptcha,
  ]);
  ...
  const executeItemPurchase = useCallback(
    async (
      ...
      buyQty: number,
      buyTicket: string,
      captchaPayload?: MarketPurchaseCaptchaTicket,
    ) => {
  ...
  const buyListing = useCallback(
    async (row: ListingItem, requestedQty: number = 1) => {
      ...
      const captchaResolution = await resolveMarketPurchaseCaptchaPayload();
      if (!captchaResolution.proceed) {
        return;
      }
      try {
        await executeItemPurchase(row.id, summary.buyQty, row.buyTicket, captchaResolution.captchaPayload);
      } catch (error) {
    ...
    [characterId, executeItemPurchase, marketPage, refreshMarket, resolveMarketPurchaseCaptchaPayload],
  );
  ...
  const buyPartnerListing = useCallback(
    async (row: PartnerListingPreviewItem) => {
      ...
      try {
        await executePartnerPurchase(row.id, row.buyTicket, captchaResolution.captchaPayload);
      } catch (error) {
        const normalizedError = toUnifiedApiError(error, '购买失败');
                    ...
                    <img className={`market-item-icon ${getItemQualityClassName(row.quality)}`} src={row.icon} alt={row.name} />
                      <div className="market-mobile-head-main">
                        ...
                        <div className="market-item-tags">
                          <Tag className={`market-tag market-tag-quality ${getItemQualityTagClassName(row.quality)}`}>{row.quality}</Tag>
                ...
                <div key={row.id} className="market-mobile-card market-mobile-card--partner">
                  ...
                  >
                    ...
                    <div className="market-mobile-head-main">
                      ...
                      <div className="market-item-tags">
                        <Tag className={`market-tag market-tag-quality ${getItemQualityTagClassName(row.partner.quality)}`}>{row.partner.quality}</Tag>
              ...
              <div className="market-bag-grid">
                ...
                {bagItems.map((b) => (
                  <InventoryItemCell
                    ...
                    qualityClassName={getItemQualityClassName(b.quality)}
                  ...
                  <>
                    <div className="market-list-detail-head">
                      <div className="market-list-detail-head-main">
                        <img className={`market-list-detail-icon ${getItemQualityClassName(selectedBagItem.quality)}`} src={selectedBagItem.icon} alt={selectedBagItem.name} />
                        <div className="market-list-detail-meta">
                          ...
                          <div className="market-list-detail-tags">
                            <Tag className={`market-tag market-tag-quality ${getItemQualityTagClassName(selectedBagItem.quality)}`}>{selectedBagItem.quality}</Tag>
                    ...
                    <div className="market-list-detail-head">
                      <div className="market-list-detail-head-main">
                        ...
                        <div className="market-list-detail-meta">
                          ...
                          <div className="market-list-detail-tags">
                            <Tag className={`market-tag market-tag-quality ${getItemQualityTagClassName(selectedPartner.quality)}`}>{selectedPartner.quality}</Tag>
